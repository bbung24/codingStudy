...
the total prime numbers up to n would be O(n^2). Could we do better?
...
5. 4 is not a prime because it is divisible by 2, which means all multiples of 4 must also be divisible 
by 2 and were already marked off. So we can skip 4 immediately and go to the next number, 5. 
...
6. In fact, we can mark off multiples of 5 starting at 5 × 5 = 25, because 5 × 2 = 10 was already marked off by 
...
if __name__ == "__main__":
    test = Solution()
    result = test.countPrimes(10)
    # Should print 4
    print result
    result = test.countPrimes(499979)
    # Should print 41537
    print result